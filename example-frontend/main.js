...
let showAlbumTracks = async (playlistLink, id, artist, album, year, cover) => {
    ...
    $(`#${id}`).after(`
        <div id="${id}-methods" style="margin-bottom: 8px;"> 
            ...
            <a id="${id}-download" style="font-size: 14px; margin-left: 30px" href="javascript:void(0)"> Download Album </a>
        </div>
            ...
            if (data[i].artists.length == 0) { // If there are no artists in the album
                ...
                Content.append(`
                    <div class="itemContainer" style="display:flex; margin: 16px;">
                        <img referrerpolicy="no-referrer" style="height:60px; width:60px;" src="${data[i].thumbnails[0].url}" alt="Album [Cover]">
                        <div style="flex-direction:column;">
                            <p style="padding-bottom: 10px; color: white"> ${data[i].artists[0].name} - ${data[i].name} </p> 
                            <a style="color: red;" id="progress-bar-${i}" href="javascript:void(0)" onclick="downloadSong('/api/download/song/${data[i].videoId}?artist=${encodeURIComponent(data[i].artists[0].name)}&title=${encodeURIComponent(data[i].name)}&album=${encodeURIComponent(data[i].album.name)}&cover=${cover}', 'progress-bar-${i}')"> DownloadMP3 </a>
                            <a style="margin-left: 10px" id="streamButton-${i}" href="javascript:void(0)" onclick="streamAudio('api/stream/song/${data[i].videoId}', '#streamButton-${i}')">Play Now</a>
                        </div>
                    </div>
                `)
            }
